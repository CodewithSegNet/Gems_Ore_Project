...
import { mockDiscounts, mockProducts, mockCategories, Discount, Product } from "../utils/mock-data";
...
export function Discounts() {
  const [discounts, setDiscounts] = useState<Discount[]>(() => {
    const stored = localStorage.getItem('gemsore_discounts');
    return stored ? JSON.parse(stored) : mockDiscounts;
  });
  ...
  useEffect(() => {
    localStorage.setItem('gemsore_discounts', JSON.stringify(discounts));
  }, [discounts]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingDiscount) {
      setDiscounts(discounts.map(discount =>
        discount.id === editingDiscount.id
          ? {
              ...
              orderMinAmount: formData.orderMinAmount ? parseFloat(formData.orderMinAmount) : undefined,
      ...
      const newDiscount: Discount = {
        id: `D${String(discounts.length + 1).padStart(3, '0')}`,
        name: formData.name,
  ...
  const toggleStatus = (id: string) => {
    setDiscounts(discounts.map(discount =>
      discount.id === id
        ? { ...discount, status: discount.status === 'active' ? 'inactive' as const : 'active' as const }
        : discount
    ));
            ...
            <CardContent>
              {filteredDiscounts.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <Button onClick={() => { resetForm(); setIsDialogOpen(true); }} className="bg-black hover:bg-slate-800">
                    <Plus className="h-4 w-4 mr-2" />
                    Create Discount
                  </Button>
                </div>
