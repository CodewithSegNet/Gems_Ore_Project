...
interface ReportData {
  revenue_trend: { month: string; sales: number }[];
...
export function FinancialReports() {
  ...
  // Fetch orders for the transactions table
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        setOrders(data.map((o: any) => ({
          ...
          paymentMethod: o.payment_method || o.paymentMethod || "N/A",
  ...
  // Fetch chart data from backend, re-fetch when dateRange changes
  useEffect(() => {
    const fetchReportData = async () => {
      setLoading(true);
      try {
        const data = await adminApi.dashboard.getFinancialReportData(dateRange);
        setReportData(data);
      } catch (e) {
  ...
  const totalOrderCount = reportData?.completed_orders ?? 0;
  const avgOrderValue = reportData?.avg_order_value ?? 0;
  const btcRevenue = reportData?.payment_breakdown?.BTC?.revenue ?? 0;
  const usdtRevenue = reportData?.payment_breakdown?.USDT?.revenue ?? 0;
  const btcCount = reportData?.payment_breakdown?.BTC?.count ?? 0;
  const usdtCount = reportData?.payment_breakdown?.USDT?.count ?? 0;
  const paystackRevenue = reportData?.payment_breakdown?.Paystack?.revenue ?? 0;

  const salesData = reportData?.revenue_trend ?? [];

  // Prepare export data
  const salesReportData = orders.map(order => ({
    'Order ID': order.id,
  ...
  const paymentReportData = [
    { Method: "BTC", Revenue: btcRevenue, Transactions: btcCount },
    { Method: "USDT", Revenue: usdtRevenue, Transactions: usdtCount },
    { Method: "Paystack", Revenue: paystackRevenue, Transactions: reportData?.payment_breakdown?.Paystack?.count ?? 0 },
  ];

  const handleExportCSV = () => {
    const data = reportType === 'sales' ? salesReportData : paymentReportData;
  ...
  const handleExportPDF = () => {
    const data = reportType === 'sales' ? salesReportData : paymentReportData;
    const title = reportType === 'sales' ? 'Sales Report' : 'Payment Analysis Report';
    const filename = `${reportType}-report-${new Date().toISOString().split('T')[0]}`;
  ...
  if (loading) {
    return (
      <div className="p-8 space-y-8">
        <div><h1 className="text-3xl font-semibold text-slate-900">Financial Reports</h1></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="border border-slate-200">
          <CardContent className="p-6 bg-white rounded-xl">
            ...
            <p className="text-sm text-slate-500 mt-2">
              {totalOrderCount} completed orders
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border border-slate-200">
          <CardContent className="p-6 bg-white rounded-xl">
            ...
            <p className="text-3xl font-semibold text-slate-900">{reportData?.total_orders ?? 0}</p>
            <p className="text-sm text-slate-500 mt-2">
              {totalOrderCount} completed
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border border-slate-200">
          <CardContent className="p-6 bg-white rounded-xl">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm text-slate-600">Crypto Revenue</p>
              <Percent className="h-5 w-5 text-slate-400" />
            </div>
            <p className="text-3xl font-semibold text-slate-900">{formatCurrency(btcRevenue + usdtRevenue)}</p>
            <p className="text-sm text-slate-500 mt-2">
              BTC + USDT combined
            </p>
          </CardContent>
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-end">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleExportPDF} className="bg-black text-white hover:bg-slate-800">
                <Download className="h-4 w-4 mr-2" />
                Export PDF
              </Button>
            </div>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <Card className="border-0 shadow-sm bg-white rounded-xl">
            <CardHeader className="pb-4">
              ...
              <p className="text-sm text-slate-500 mt-1">Monthly revenue from completed orders</p>
            </CardHeader>
            <CardContent>
              {salesData.length > 0 ? (
                <ResponsiveContainer width="100%" height={350}>
                  <LineChart data={salesData}>
                    ...
                    />
                    <Legend />
                    <Line type="monotone" dataKey="sales" stroke="#f59e0b" strokeWidth={3} name="Revenue" dot={{ r: 4 }} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
                <p className="text-center text-slate-400 py-12">No sales data available yet</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-sm bg-white rounded-xl">
            <CardHeader className="pb-4">
              <CardTitle className="text-lg font-semibold text-slate-900">Revenue by Payment Method</CardTitle>
              <p className="text-sm text-slate-500 mt-1">Breakdown of completed order revenue by payment type</p>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={[
                  { name: "BTC", revenue: btcRevenue, count: btcCount },
                  { name: "USDT", revenue: usdtRevenue, count: usdtCount },
                  { name: "Paystack", revenue: paystackRevenue, count: reportData?.payment_breakdown?.Paystack?.count ?? 0 },
                ]}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false} />
                  ...
                  <TableBody>
                    {orders.map((order) => (
                      <TableRow key={order.id} className="border-slate-100">
                        <TableCell className="font-mono text-slate-600">{order.id.slice(0, 8)}...</TableCell>
                  ...
                  <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl">
                    <div className="flex justify-between items-center mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-slate-900">Paystack</span>
                      </div>
                      <span className="font-semibold text-slate-900">{formatCurrency(paystackRevenue)}</span>
                    </div>
                    <div className="text-sm text-slate-600">
                      {reportData?.payment_breakdown?.Paystack?.count ?? 0} transactions
                    </div>
                  </div>
                </div>
                <div className="pt-4 border-t border-slate-100">
                  <div className="flex justify-between items-center">
                    <span className="font-semibold text-slate-900">Total Crypto Revenue</span>
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl">
                  ...
                  <span className="font-semibold text-slate-900">
                    {totalRevenue > 0 ? ((btcRevenue / totalRevenue) * 100).toFixed(1) : '0.0'}%
                  </span>
                </div>
                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl">
                  <span className="text-slate-600 font-medium">USDT Percentage</span>
                  <span className="font-semibold text-slate-900">
                    {totalRevenue > 0 ? ((usdtRevenue / totalRevenue) * 100).toFixed(1) : '0.0'}%
                  </span>
                </div>
                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl">
                  <span className="text-slate-600 font-medium">Avg BTC Transaction</span>
                  <span className="font-semibold text-slate-900">
                    {formatCurrency(btcCount > 0 ? btcRevenue / btcCount : 0)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl">
                  <span className="text-slate-600 font-medium">Avg USDT Transaction</span>
                  <span className="font-semibold text-slate-900">
                    {formatCurrency(usdtCount > 0 ? usdtRevenue / usdtCount : 0)}
                  </span>
                </div>
              </CardContent>
            </Card>
